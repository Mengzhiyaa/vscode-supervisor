...
export interface ANSIOutputLine {
	 ...
	 */
	readonly outputRuns: readonly ANSIOutputRun[];
}
...
export class ANSIOutput {
	...
	/**
	 * Permanently truncates the internal output lines to the specified maximum.
	 * This frees memory by removing old lines that exceed the scrollback limit.
	 * @param maxOutputLines The maximum number of output lines to keep.
	 */
	truncate(maxOutputLines: number): void {
		// If the max output lines is less than or equal to zero, clear all lines.
		if (maxOutputLines <= 0) {
			this._outputLines = [];
			this._outputLine = 0;
			this._outputColumn = 0;
		...
		// If there are fewer output lines than the max, nothing to truncate.
		if (this._outputLines.length <= maxOutputLines) {
			return;
		}

		// Calculate how many lines to remove.
		const linesToRemove = this._outputLines.length - maxOutputLines;

		// Keep only the last maxOutputLines.
		this._outputLines = this._outputLines.slice(-maxOutputLines);

		// Adjust the output line cursor if needed.
		this._outputLine = Math.max(0, this._outputLine - linesToRemove);
	}
	...
	private flushBuffer() {
		...
		if (this._buffer) {
			...
			const updatedLine = outputLine.insert(
				this._buffer,
				this._outputColumn,
				this._sgrState,
				this._hyperlink
			);
			if (updatedLine !== outputLine) {
				this._outputLines[this._outputLine] = updatedLine;
			}
	...
	private processED() {
		...
		if (match) {
			...
			switch (getParam(match[1], 0)) {
				...
				case 0: {
					this._outputLines[this._outputLine] =
					...
					for (let i = this._outputLine + 1; i < this._outputLines.length; i++) {
						this._outputLines[i] = this._outputLines[i].clearEntireLine();
					}
					break;
				}
	...
	private processEL() {
		...
		if (match) {
			// Get the output line.
			const outputLine = this._outputLines[this._outputLine];
			let updatedLine = outputLine;

			// Process the parameter.
			switch (getParam(match[1], 0)) {
				// Clear from cursor to the end of the line.
				case 0: {
					updatedLine = outputLine.clearToEndOfLine(this._outputColumn);
					break;
				}

				// Clear from cursor to the beginning of the line.
				case 1: {
					updatedLine = outputLine.clearToBeginningOfLine(this._outputColumn);
					break;
				}

				// Clear the entire line.
				case 2: {
					updatedLine = outputLine.clearEntireLine();
					break;
				}
			}
...
class OutputLine implements ANSIOutputLine {
	...
	/**
	 * Constructor.
	 * @param outputRuns The output runs.
	 * @param totalLength The total length.
	 * @param id The identifier.
	 */
	constructor(outputRuns: OutputRun[] = [], totalLength?: number, id: string = generateId()) {
		this._outputRuns = outputRuns;
		this._totalLength = totalLength ?? outputRuns.reduce((total, run) => total + run.text.length, 0);
		this._id = id;
	}
	...
	private withRuns(outputRuns: OutputRun[], totalLength?: number): OutputLine {
		return new OutputLine(
			outputRuns,
			totalLength ?? outputRuns.reduce((total, run) => total + run.text.length, 0),
			this._id
		);
	}
	 ...
	 */
	public clearEntireLine(): OutputLine {
		// If there are output runs, replace them all with an empty output run.
		if (!this._totalLength) {
			return this;
		}

		return this.withRuns([new OutputRun(' '.repeat(this._totalLength))], this._totalLength);
	}
	 ...
	 */
	public clearToEndOfLine(column: number): OutputLine {
		...
		if (!leftTextLength) {
			...
			const leftRun = leftTextLength === leftOutputRun.text.length
				? leftOutputRun
				: leftOutputRun.withText(leftText);
			outputRuns.push(leftRun);
			outputRuns.push(new OutputRun(erasureText));
		}

		// Splice the new output runs in.
		const updatedRuns = [
			...this._outputRuns.slice(0, leftOutputRunIndex),
			...outputRuns
		];

		return this.withRuns(updatedRuns, this._totalLength);
	}
	 ...
	 * @param column The column at which to clear from.
	 */
	public clearToBeginningOfLine(column: number): OutputLine {
		// Sanity check the column.
		column = Math.max(column, 0);
		...
		// Splice the new output runs in.
		const deleteCount = this._outputRuns.length - rightOutputRunIndex;
		const updatedRuns = [
			...outputRuns,
			...this._outputRuns.slice(deleteCount)
		];
	 ...
	 */
	public insert(text: string, column: number, sgrState?: SGRState, hyperlink?: Hyperlink): OutputLine {
		...
		if (column === this._totalLength) {
			...
			const totalLength = this._totalLength + text.length;
			...
			if (this._outputRuns.length) {
				...
				if (SGRState.equivalent(lastOutputRun.sgrState, sgrState) &&
					...
					const updatedRuns = [
						...this._outputRuns.slice(0, -1),
						lastOutputRun.withText(lastOutputRun.text + text)
					];

					// The insert is complete.
					return this.withRuns(updatedRuns, totalLength);
				}
			}

			// Append a new output run for the text being inserted.
			return this.withRuns(
				[...this._outputRuns, new OutputRun(text, sgrState, hyperlink)],
				totalLength
			);
		}
		...
		if (column > this._totalLength) {
			...
			const totalLength = this._totalLength + spacer.length + text.length;
			...
			if (this._outputRuns.length) {
				...
				if (SGRState.equivalent(lastOutputRun.sgrState, sgrState) &&
					...
					const updatedRuns = [
						...this._outputRuns.slice(0, -1),
						lastOutputRun.withText(lastOutputRun.text + spacer + text)
					];
			...
			return this.withRuns(
				[
					...this._outputRuns,
					new OutputRun(spacer),
					new OutputRun(text, sgrState, hyperlink)
				],
				totalLength
			);
		}
		...
		if (column + text.length >= this._totalLength) {
			...
			if (!leftTextLength) {
				...
				if (SGRState.equivalent(leftOutputRun.sgrState, sgrState) &&
					...
					outputRuns.push(leftOutputRun.withText(leftText + text));
				} else {
					// Push an output run for the left text.
					outputRuns.push(leftOutputRun.withText(leftText));
			...
			const totalLength = leftOffset + leftTextLength + text.length;
			const updatedRuns = [
				...this._outputRuns.slice(0, leftOutputRunIndex),
				...outputRuns
			];
		...
		if (leftOutputRunTextLength) {
			...
			outputRuns.push(leftOutputRun.withText(leftOutputRunText));
		}
		...
		if (rightOutputRunTextLength) {
			...
			const rightRun = rightOutputRunIndex !== leftOutputRunIndex
				? rightOutputRun.withText(rightOutputRunText)
				: new OutputRun(rightOutputRunText, rightOutputRun.sgrState, rightOutputRun.hyperlink);
			outputRuns.push(rightRun);
		}
		...
		const updatedRuns = [
			...this._outputRuns.slice(0, leftOutputRunIndex),
			...outputRuns,
			...this._outputRuns.slice(rightOutputRunIndex + 1)
		];

		// Optimize the output runs.
		const optimizedRuns = updatedRuns.length > 1
			? OutputRun.optimizeOutputRuns(updatedRuns)
			: updatedRuns;

		// Recalculate the total length.
		const totalLength = optimizedRuns.reduce((total, outputRun) =>
			total + outputRun.text.length,
			0
		);

		return this.withRuns(optimizedRuns, totalLength);
	}
	 ...
	 * Gets the output runs.
	 */
	public get outputRuns(): readonly ANSIOutputRun[] {
...
class OutputRun implements ANSIOutputRun {
	...
	public static optimizeOutputRuns(outputRuns: OutputRun[]) {
		...
		for (let right = 1, left = 0; right < outputRuns.length; right++) {
			...
			const leftOutputRun = optimizedOutputRuns[left];
			...
			if (SGRState.equivalent(leftOutputRun.sgrState, rightOutputRun.sgrState) &&
				...
				optimizedOutputRuns[left] = leftOutputRun.withText(
					leftOutputRun.text + rightOutputRun.text
				);
			} else {
	...
	/**
	 * Returns a copy of the output run with new text.
	...
	withText(text: string) {
		...
		const outputRun = new OutputRun(text, this._sgrState, this._hyperlink);
		outputRun._id = this._id;
		return outputRun;
	}
