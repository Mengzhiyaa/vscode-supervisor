...
<script lang="ts">
    ...
    import ColumnNullPercent from "./columnNullPercent.svelte";
    ...
    const isInteger = $derived(isIntegerDisplayType(resolvedProfileType));
    ...
    const isDateOnly = $derived(isDateDisplayType(resolvedProfileType));
    const isDateTime = $derived(isDatetimeDisplayType(resolvedProfileType));
    const isObject = $derived(isObjectDisplayType(resolvedProfileType));
    ...
    function handleDoubleClick() {
        ...
        const tableDataDataGridInstance = context.tableDataDataGridInstance;
        if (tableDataDataGridInstance) {
            tableDataDataGridInstance.setCursorPosition(columnIndex, 0);
        }
    }
...
>
    ...
    {#if expanded}
        <div class="column-profile-info">
            {#if isNumeric}
                {#if isInteger}
                    <ColumnProfileInteger
                        instance={context.tableSchemaDataGridInstance}
                        {columnIndex}
                    />
                {:else}
                    <ColumnProfileNumber
...
<style>
    ...
    .column-summary
        .column-profile-info
        :global(.column-profile-expanded.column-profile-info) {
        display: grid;
        grid-template-rows:
            [sparkline] min-content
            [tabular-info] min-content
    ...
    .column-summary .column-profile-info :global(.column-profile-sparkline) {
        margin-bottom: 10px;
        grid-row: sparkline / tabular-info;
    }

    .column-summary .column-profile-info :global(.tabular-info) {
        display: grid;
        margin: 0 auto;
        min-width: 200px;
        overflow: hidden;
    ...
    .column-summary .column-profile-info :global(.tabular-info .labels) {
        margin-right: 10px;
        grid-column: labels / spacer;
    }

    .column-summary .column-profile-info :global(.tabular-info .labels .label) {
        height: 20px;
    }

    .column-summary .column-profile-info :global(.tabular-info .spacer) {
        grid-column: spacer / values;
    }

    .column-summary .column-profile-info :global(.tabular-info .values) {
        overflow: hidden;
        text-align: right;
        grid-column: values / end-columns;
    }

    .column-summary .column-profile-info :global(.tabular-info .values .value) {
        height: 20px;
        font-weight: 600;
        overflow: hidden;
        text-wrap: nowrap;
    ...
    .column-summary
        .column-profile-info
        :global(.tabular-info .values .value-placeholder) {
        height: 20px;
    }
