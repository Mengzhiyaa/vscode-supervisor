<!--
  dataGridRowCell.svelte - Individual data cell (Svelte 5 runes mode)
...
<script lang="ts">
    ...
    import {
        DataCellKind,
        type DataCell,
    } from "../../dataExplorer/common/tableDataCache";
    ...
    function getDataCell(
        ...
        if (
            ...
            "getDataCell" in candidate &&
            typeof candidate.getDataCell === "function"
        ) {
            return candidate.getDataCell(rowIndex, columnIndex) as
                | DataCell
                | undefined;
        }
    ...
    const dataCell = $derived.by(() => {
        $updateTrigger;
        return getDataCell(instance, rowIndex, columnIndex);
    });
