...
import { PlotRenderFormat, type IntrinsicSize, type PlotMetadata } from './comms/positronPlotComm';

/**
 * The type of operation being queued.
 * Matches Positron's OperationType enum.
 */
 ...
 * Matches Positron's PlotOperationResult type.
 */
 ...
 * Matches Positron's PlotOperationRequest interface.
 */
 ...
 * Matches Positron's RenderRequest interface.
 */
 ...
 * Matches VS Code's DeferredPromise pattern.
 */
 ...
 *
 * Matches Positron's DeferredPlotOperation class.
 */
 ...
 *
 * Matches Positron's DeferredRender class.
 */
...
/**
 * Interface for plot comm that can perform RPC operations.
 * Matches the interface expected by PositronPlotRenderQueue.
 */
export interface IPlotComm {
    readonly clientId: string;
    onDidClose: vscode.Event<void>;
    render(size: IPlotSize | undefined, pixelRatio: number, format: PlotRenderFormat): Promise<{
        data: string;
        mime_type: string;
    }>;
 ...
 * Matches Positron's QueuedOperation class.
 */
export class QueuedOperation {
    constructor(
        public readonly operation: DeferredPlotOperation,
        public readonly comm: IPlotComm) {
    }
}

/**
 * Queued render wrapper.
 * Matches Positron's QueuedRender class.
 */
...
/**
 * Runtime state enum matching Positron's RuntimeState.
 */
...
/**
 * Interface for a runtime session that can provide state information.
 */
 ...
 * Matches Positron's padBase64 function.
 */
 ...
 *
 * Matches Positron's PositronPlotRenderQueue class (569 lines).
 */
export class PositronPlotRenderQueue implements vscode.Disposable {
     ...
     */
    private readonly _commCloseListeners = new Map<string, vscode.Disposable>();
    ...
    constructor(
        private readonly _session: IRuntimeSession
    ) {
        this._outputChannel = vscode.window.createOutputChannel(`Plot Render Queue [${_session.sessionId}]`, { log: true });
    ...
    public queueOperation(request: PlotOperationRequest, comm: IPlotComm): DeferredPlotOperation {
        ...
        const state = this._session.getRuntimeState();
        if (state === RuntimeState.Idle || state === RuntimeState.Ready) {
            this.processQueue();
        }
     ...
     *
     * @param deferredRender The render request to queue
     * @param comm The comm to use for the operation
     */
    public queue(deferredRender: DeferredRender, comm: IPlotComm): DeferredRender {
     ...
     * @param comm The comm to use for the operation
     */
    public queueIntrinsicSizeRequest(comm: IPlotComm): Promise<IntrinsicSize | undefined> {
     ...
     * @param comm The comm to use for the operation
     */
    public queueMetadataRequest(comm: IPlotComm): Promise<PlotMetadata> {
     ...
     */
    private cancelExistingOperations(comm: IPlotComm, operationType: OperationType): void {
    ...
    private processQueue(): void {
        ...
        if (operationRequest.type === OperationType.Render) {
            ...
            queuedOperation.comm.render(
                operationRequest.size,
                operationRequest.pixel_ratio!,
                operationRequest.format!
            ).then((response) => {
     ...
     */
    private ensureCommCloseListener(comm: IPlotComm): void {
