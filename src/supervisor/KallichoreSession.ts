...
export class KallichoreSession implements JupyterLanguageRuntimeSession {
	...
	/** A map of pending LSP comm startups keyed by the client comm id */
	private _startingComms: Map<string, PromiseHandles<number>> = new Map();
	private _lspClientRegistrations: Map<string, vscode.Disposable> = new Map();
	...
	async buildEnvVarActions(restart: boolean): Promise<VarAction[]> {
		...
		// spec-provided entries such as R_HOME or the selected Python path.
	...
	async startPositronLsp(clientId: string, ipAddress: string): Promise<number> {
		...
		const existingRegistration = this._lspClientRegistrations.get(clientId);
		existingRegistration?.dispose();
		const registration = positron.runtime.registerClientInstance(clientId);
		this._lspClientRegistrations.set(clientId, registration);
		...
		try {
			...
			this._startingComms.delete(clientId);
			registration.dispose();
			this._lspClientRegistrations.delete(clientId);
			throw error;
		}
	...
	async createServerComm(target_name: string, ip_address: string): Promise<[Comm, number]> {
		this.log(`Starting server comm '${target_name}' for ${ip_address}`, vscode.LogLevel.Debug);
		...
		this.log(`Started server comm '${target_name}' for ${ip_address} on port ${port}`, vscode.LogLevel.Debug);
		return [comm, port];
	}
	...
	callMethod(method: string, ...args: Array<any>): Promise<any> {
		return this._callUiCommRequest(
			method,
			'call_method',
			{
				method,
	...
	/**
	 * Calls a method on the UI comm using the given JSON-RPC method name.
	 *
	 * @param method The JSON-RPC method name to call on the UI comm
	 * @param params Optional parameters to pass to the method
	 * @returns The result of the method call
	 */
	callUiComm(method: string, params?: Record<string, unknown>): Promise<any> {
		return this._callUiCommRequest(method, method, params);
	}

	/**
	 * Evaluate a code expression silently and return the result as JSON.
	 *
	 * This sends an `evaluate_code` request to the UI comm which executes
	 * the code silently and returns the result + output.
	 *
	 * @param code The code expression to evaluate
	 * @returns The evaluation result
	 */
	async evaluate(code: string): Promise<import('./positron-supervisor').EvaluateCodeResult> {
		await this.waitForIdle();

		const result = await this.callUiComm('evaluate_code', { code });
		return {
	...
	/**
	 * Get information about how the runtime was launched.
	 *
	 * @returns Launch information from the kernel spec or restored session metadata
	 */
	async getLaunchInfo(): Promise<import('./positron-supervisor').RuntimeLaunchInfo | undefined> {
		if (this._kernelSpec) {
	...
	/**
	 * Performs a UI comm request using the given JSON-RPC method name.
	 */
	private _callUiCommRequest(
		label: string,
		rpcMethod: string,
		...
		// Create the request
		const request = new UICommRequest(label, rpcMethod, params, promise);
		...
		if (!uiComm) {
			...
			this.log(`No UI comm open yet; queueing request '${label}'`, vscode.LogLevel.Debug);
	...
	removeClient(id: string): void {
		const startingComm = this._startingComms.get(id);
		if (startingComm) {
			this._startingComms.delete(id);
			startingComm.reject(new Error('Comm was closed before sending a `server_started` message'));
		}

		this._lspClientRegistrations.get(id)?.dispose();
		this._lspClientRegistrations.delete(id);
	...
	private async tryStart(retry: boolean): Promise<positron.LanguageRuntimeInfo> {
		...
		// If we don't have runtime info yet, or if we don't have a banner,
		// get the full kernel info now. The startSession API may return
		// status: 'ok' without the complete kernel_info_reply.
		if (!runtimeInfo || !runtimeInfo.banner) {
	...
	async waitForIdle(): Promise<void> {
		// If already idle, resolve immediately (upstream Positron improvement)
		if (this._runtimeState === positron.RuntimeState.Idle) {
			return;
		}

		return new Promise((resolve, _reject) => {
			const disposable = this._state.event((state) => {
	...
	handleKernelMessage(data: any) {
		...
		if (data.hasOwnProperty('status')) {
			...
			if (Object.values(positron.RuntimeState).includes(status)) {
				...
				this.log(`Unknown state: ${status}`, vscode.LogLevel.Warning);
			}
	...
	private onStateChange(newState: positron.RuntimeState, reason: string) {
		if (newState === positron.RuntimeState.Ready) {
			// We're ready now!
			this.log(`Kernel is ready.`, vscode.LogLevel.Debug);
		}
	...
	async handleJupyterMessage(data: any) {
		...
		switch (msg.header.msg_type) {
			case 'comm_close': {
				...
				const startingPromise = this._startingComms.get(closeMsg.comm_id);
				if (startingPromise) {
					this._startingComms.delete(closeMsg.comm_id);
					startingPromise.reject(new Error('Comm was closed before sending a `server_started` message'));
				}
		...
		if (msg.header.msg_type === 'comm_msg') {
			...
			if (commMsg.data?.msg_type === 'server_started') {
				const serverStarted = commMsg.data.content as { port?: unknown };
				const startingPromise = this._startingComms.get(commMsg.comm_id);
				if (startingPromise) {
					if (typeof serverStarted.port === 'number') {
						startingPromise.resolve(serverStarted.port);
					} else {
						startingPromise.reject(new Error('`server_started` message doesn\'t include a port'));
					}
