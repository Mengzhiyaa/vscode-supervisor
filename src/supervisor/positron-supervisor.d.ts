...
// Local positron compatibility layer
...
export interface JupyterLanguageRuntimeSession extends positron.LanguageRuntimeSession {
	...
	/**
	 * Call a UI comm method by JSON-RPC method name.
	 */
	 ...
	 * - Always executes silently (never visible in the Console)
	 * - Returns the structured value of the expression, not just printed output
	 * - Returns the value as JSON, not as text
	 *
	 * @param code The code expression to evaluate
	 * @returns The evaluation result including structured value and output
	 */
	 ...
	 *
	 * @returns Launch information including kernel argv/env metadata, if available
	 */
...
/**
 * The result of evaluating a code expression silently.
 */
export interface EvaluateCodeResult {
	/** The evaluated value as a JSON-compatible type. */
	result: any;

	/** The text output produced during evaluation, if any. */
	output: string;
}

/**
 * Information about how a runtime session was launched.
 */
export interface RuntimeLaunchInfo {
	/** The command line used to start the kernel */
	argv: string[];

	/** Environment variables that were set for the kernel */
