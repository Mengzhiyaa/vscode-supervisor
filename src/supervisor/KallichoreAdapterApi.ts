...
export class KCApi implements PositronSupervisorApi {
	...
	constructor(
		...
		this.ensureStarted().then(async () => {
			...
			this.log(`Failed to start Kallichore server: ${err}`, vscode.LogLevel.Error);
		});
	}
	...
	async start() {
		...
		if (connectionFile) {
			if (fs.existsSync(connectionFile)) {
				this.log(`Using connection file from ` +
					`POSITRON_SUPERVISOR_CONNECTION_FILE: ${connectionFile}`, vscode.LogLevel.Debug);
				try {
					const connectionContents = JSON.parse(fs.readFileSync(connectionFile, 'utf8'));
					if (await this.reconnect(connectionContents)) {
						this.log(
							`Connected to previously established supervisor.`, vscode.LogLevel.Debug);
						return;
					}
					...
					// is invalid.
					this.log(
						`Error connecting to Kallichore (${connectionFile}): ${summarizeError(err)}`, vscode.LogLevel.Warning);
				}
			} else {
				// Non-fatal, but not expected.
				this.log(`Connection file named in ` +
					`POSITRON_SUPERVISOR_CONNECTION_FILE does not exist: ${connectionFile}`, vscode.LogLevel.Warning);
			}
		}
			...
			try {
				if (await this.reconnect(serverState)) {
					...
					this.log(`Could not reconnect to Kallichore server ` +
						`at ${connectionInfo}. Starting a new server`, vscode.LogLevel.Debug);
				}
			} catch (err) {
				const connectionInfo = serverState.base_path ||
					(serverState.socket_path ? `socket:${serverState.socket_path}` : '') ||
				...
				this.log(`Failed to reconnect to Kallichore server ` +
					` at ${connectionInfo}: ${err}. Starting a new server.`, vscode.LogLevel.Warning);
			}
		}
		...
		if (shutdownTimeout !== 'immediately') {
			...
			if (os.platform() === 'win32') {
				...
				this.log(`Running Kallichore server with PowerShell detached process to persist sessions`, vscode.LogLevel.Debug);
				...
				this.log(`Running Kallichore server with nohup to persist sessions`, vscode.LogLevel.Debug);
		...
		this._disposables.push(new vscode.Disposable(() => {
			try {
				...
				this.log(`Error cleaning up output file ${outFile}: ${err}`, vscode.LogLevel.Warning);
			}
		}));
		...
		this._logStreamer.watch().then(() => {
			this.log(`Streaming Kallichore server logs from ${logFile} (log level: ${logLevel})`, vscode.LogLevel.Debug);
		});
	...
	getShutdownHours(): number {
		...
		if (shutdownTimeout === 'immediately') {
			...
			try {
				...
				// it if it does.
				this.log(`Invalid hour value for kernelSupervisor.shutdownTimeout: '${shutdownTimeout}'; persisting sessions indefinitely`, vscode.LogLevel.Warning);
			}
		}
	...
	async reconnect(serverState: KallichoreServerState): Promise<boolean> {
		...
		if (pid) {
			try {
				...
				this.log(`Kallichore server PID ${pid} is not running`, vscode.LogLevel.Debug);
				return false;
			}
		}
		...
		this._logStreamer.watch().then(() => {
			this.log(`Streaming Kallichore server logs at ${serverState.log_path}`, vscode.LogLevel.Debug);
		});
	...
	async updateIdleTimeout() {
		...
		try {
			...
			this.log(`Failed to update idle timeout: ${summarizeError(err)}`, vscode.LogLevel.Warning);
		}
	}
				...
				this._api.api.clientHeartbeat(heartbeatPayload).catch(async (err) => {
					if (err.code === 'ECONNREFUSED' || err.code === 'ENOENT') {
						...
						this.log(
							...
							`checking server status`, vscode.LogLevel.Warning);
						await this.testServerExited();
					} else {
						this.log(`Failed to send client heartbeat: ` +
							summarizeError(err), vscode.LogLevel.Warning);
					}
				});
			} else {
	...
	async createSession(
		...
		this.log(`Creating session: ${JSON.stringify(sessionMetadata)}`, vscode.LogLevel.Debug);
		...
		this._disposables.push(session.disconnected.event(async (evt: DisconnectedEvent) => {
			if (evt.reason === DisconnectReason.Unknown) {
				...
				this.log(`Session '${session.metadata.sessionId}' disconnected ` +
					`while in state '${evt.state}'. This is unexpected; checking server status.`, vscode.LogLevel.Warning);
				...
				if (!exited) {
					this.log(`The server is still running; attempting to reconnect to session ${session.metadata.sessionId}`, vscode.LogLevel.Debug);
					try {
						await withTimeout(session.connect(), 2000, `Timed out reconnecting to session ${session.metadata.sessionId}`);
						this.log(`Successfully restored connection to  ${session.metadata.sessionId}`, vscode.LogLevel.Debug);
					} catch (err) {
				...
				this.log(`Session '${session.metadata.sessionId}' disconnected ` +
					`because another client connected to it.`, vscode.LogLevel.Warning);
	...
	private async testServerExited(): Promise<boolean> {
		...
		if (!serverState) {
			this.log(`No Kallichore server state found; cannot test server process`, vscode.LogLevel.Debug);
			return false;
		}
		...
		if (serverState.server_pid) {
			try {
				...
				this.log(`Kallichore server PID ${serverState.server_pid} is still running`, vscode.LogLevel.Debug);
			} catch (err) {
				this.log(`Kallichore server PID ${serverState.server_pid} is not running`, vscode.LogLevel.Warning);
				serverRunning = false;
			}
		}
		...
		for (const session of this._sessions) {
			if (session.runtimeState !== positron.RuntimeState.Exited) {
				hadAbend = true;
				this.log(`Marking session ${session.metadata.sessionId} as exited because the server is no longer running`, vscode.LogLevel.Warning);
	...
	async validateSession(sessionId: string): Promise<boolean> {
		...
		try {
			...
			this.log(`Error validating session ${sessionId}: ${summarizeError(e)}`, vscode.LogLevel.Warning);
		}
		...
		return new Promise<JupyterLanguageRuntimeSession>((resolve, reject) => {
			this._api.api.getSession(sessionMetadata.sessionId).then(async (response) => {
				...
				if (kcSession.status === Status.Exited) {
					this.log(`Attempt to reconnect to session ${sessionMetadata.sessionId} failed because it is no longer running`, vscode.LogLevel.Warning);
				...
				try {
					...
					this.log(`Failed to restore session ${sessionMetadata.sessionId}: ${JSON.stringify(err)}`, vscode.LogLevel.Error);
					if (err.code === 'ECONNREFUSED') {
						this.log(`Connection refused while attempting to restore session; checking server status`, vscode.LogLevel.Warning);
						await this.testServerExited();
					}
					if (err.code === 'ENOENT') {
						this.log(`Socket/pipe not found while attempting to restore session; checking server status`, vscode.LogLevel.Warning);
						await this.testServerExited();
					}
					reject(err);
				}
				...
				if (isAxiosError(err)) {
					...
					this.log(`Failed to reconnect to session ${sessionMetadata.sessionId}: ${message}`, vscode.LogLevel.Error);
					reject(message);
					return;
				}
				this.log(`Failed to reconnect to session ${sessionMetadata.sessionId}: ${JSON.stringify(err)}`, vscode.LogLevel.Error);
				reject(err);
			});
		});
	}
	...
	getKallichorePath(): string {
		...
		if (devBinary) {
			this.log(`Loading Kallichore from disk in adjacent repository (${devBinary}). Make sure it's up-to-date.`, vscode.LogLevel.Debug);
			return devBinary;
		}
	...
	async reconnectActiveSession() {
		...
		const sessionName = dynState?.sessionName ?? session.metadata.sessionId;
